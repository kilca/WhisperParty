...
namespace FreeDraw
{
    ...
    public class MultDrawable : Photon.MonoBehaviour, IPunObservable
    {
        // PEN COLOUR
        ...
        public static MultDrawable drawable;
        ...
        void Update()
        {
            ...
            if (mouse_held_down && !no_drawing_on_current_drag)
            {
                ...
                if (hit != null && hit.transform != null)
                {
                    ...
                    mousePos.Add(mouse_world_position);
        ...
        void Awake()
        {
            ...
            idNumber = PhotonNetwork.countOfPlayers - 1;
            if (idNumber < 0)
                idNumber = 0;

            this.GetComponent<SpriteRenderer>().sprite = sprites[idNumber];
        ...
        public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
        {
            ...
            if (stream.isWriting)
            {
                stream.SendNext(mousePos.ToArray());
                mousePos.Clear();
            }
            else
            {
                Vector2[] mousePos = (Vector2[])stream.ReceiveNext();

                foreach(Vector2 v in mousePos) {
                    current_brush(v);
                }
            }
        }
